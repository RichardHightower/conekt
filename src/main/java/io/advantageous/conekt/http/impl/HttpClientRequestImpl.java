...
public class HttpClientRequestImpl implements HttpClientRequest {
    ...
    public HttpClientRequestImpl setChunked(boolean chunked) {
        synchronized (getLock()) {
            ...
            if (client.getOptions().getProtocolVersion() != io.advantageous.conekt.http.HttpVersion.HTTP_1_0) {
    ...
    @Override
    public io.advantageous.conekt.http.HttpMethod method() {
        return method;
    }
    ...
    public void end(Buffer chunk) {
        synchronized (getLock()) {
            ...
            if (!chunked && !contentLengthSet()) {
                headers().set(io.advantageous.conekt.http.HttpHeaders.CONTENT_LENGTH, String.valueOf(chunk.length()));
            }
    ...
    private Handler<HttpClientResponse> checkConnect(io.advantageous.conekt.http.HttpMethod method, Handler<HttpClientResponse> handler) {
    ...
    private boolean contentLengthSet() {
        return headers != null && request.headers().contains(io.advantageous.conekt.http.HttpHeaders.CONTENT_LENGTH);
    }
    ...
    private void prepareHeaders() {
        ...
        if (client.getOptions().isTryUseCompression() && request.headers().get(io.advantageous.conekt.http.HttpHeaders.ACCEPT_ENCODING) == null) {
            // if compression should be used but nothing is specified by the user support deflate and gzip.
            request.headers().set(io.advantageous.conekt.http.HttpHeaders.ACCEPT_ENCODING, io.advantageous.conekt.http.HttpHeaders.DEFLATE_GZIP);
        }
        if (!client.getOptions().isKeepAlive() && client.getOptions().getProtocolVersion() == io.advantageous.conekt.http.HttpVersion.HTTP_1_1) {
            request.headers().set(io.advantageous.conekt.http.HttpHeaders.CONNECTION, io.advantageous.conekt.http.HttpHeaders.CLOSE);
        } else if (client.getOptions().isKeepAlive() && client.getOptions().getProtocolVersion() == io.advantageous.conekt.http.HttpVersion.HTTP_1_0) {
            request.headers().set(io.advantageous.conekt.http.HttpHeaders.CONNECTION, io.advantageous.conekt.http.HttpHeaders.KEEP_ALIVE);
        }
    }
    ...
    private HttpVersion toNettyHttpVersion(io.advantageous.conekt.http.HttpVersion version) {
        switch (version) {
            case HTTP_1_0: {
