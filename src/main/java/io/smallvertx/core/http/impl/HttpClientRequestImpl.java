...
public class HttpClientRequestImpl implements HttpClientRequest {
    ...
    private final io.smallvertx.core.http.HttpMethod method;
    ...
    HttpClientRequestImpl(HttpClientImpl client, io.smallvertx.core.http.HttpMethod method, String host, int port,
    ...
    public HttpClientRequestImpl setChunked(boolean chunked) {
        synchronized (getLock()) {
            ...
            if (client.getOptions().getProtocolVersion() != io.smallvertx.core.http.HttpVersion.HTTP_1_0) {
    ...
    @Override
    public io.smallvertx.core.http.HttpMethod method() {
        return method;
    }
    ...
    public void end(Buffer chunk) {
        synchronized (getLock()) {
            ...
            if (!chunked && !contentLengthSet()) {
                headers().set(io.smallvertx.core.http.HttpHeaders.CONTENT_LENGTH, String.valueOf(chunk.length()));
            }
    ...
    private Handler<HttpClientResponse> checkConnect(io.smallvertx.core.http.HttpMethod method, Handler<HttpClientResponse> handler) {
        if (method == io.smallvertx.core.http.HttpMethod.CONNECT) {
    ...
    private boolean contentLengthSet() {
        return headers != null && request.headers().contains(io.smallvertx.core.http.HttpHeaders.CONTENT_LENGTH);
    }
    ...
    private void prepareHeaders() {
        ...
        headers.remove(io.smallvertx.core.http.HttpHeaders.TRANSFER_ENCODING);
        if (!headers.contains(io.smallvertx.core.http.HttpHeaders.HOST)) {
            request.headers().set(io.smallvertx.core.http.HttpHeaders.HOST, conn.hostHeader());
        }
        if (chunked) {
            HttpHeaders.setTransferEncodingChunked(request);
        }
        if (client.getOptions().isTryUseCompression() && request.headers().get(io.smallvertx.core.http.HttpHeaders.ACCEPT_ENCODING) == null) {
            // if compression should be used but nothing is specified by the user support deflate and gzip.
            request.headers().set(io.smallvertx.core.http.HttpHeaders.ACCEPT_ENCODING, io.smallvertx.core.http.HttpHeaders.DEFLATE_GZIP);
        }
        if (!client.getOptions().isKeepAlive() && client.getOptions().getProtocolVersion() == io.smallvertx.core.http.HttpVersion.HTTP_1_1) {
            request.headers().set(io.smallvertx.core.http.HttpHeaders.CONNECTION, io.smallvertx.core.http.HttpHeaders.CLOSE);
        } else if (client.getOptions().isKeepAlive() && client.getOptions().getProtocolVersion() == io.smallvertx.core.http.HttpVersion.HTTP_1_0) {
            request.headers().set(io.smallvertx.core.http.HttpHeaders.CONNECTION, io.smallvertx.core.http.HttpHeaders.KEEP_ALIVE);
        }
    }
    ...
    private HttpMethod toNettyHttpMethod(io.smallvertx.core.http.HttpMethod method) {
        switch (method) {
            case CONNECT: {
    ...
    private HttpVersion toNettyHttpVersion(io.smallvertx.core.http.HttpVersion version) {
        switch (version) {
            case HTTP_1_0: {
