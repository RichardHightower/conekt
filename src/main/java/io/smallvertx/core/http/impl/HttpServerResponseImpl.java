...
public class HttpServerResponseImpl implements HttpServerResponse {
    ...
    HttpServerResponseImpl(final VertxInternal vertx, ServerConnection conn, HttpRequest request) {
        ...
        this.keepAlive = (version == HttpVersion.HTTP_1_1 && !request.headers().contains(io.smallvertx.core.http.HttpHeaders.CONNECTION, io.smallvertx.core.http.HttpHeaders.CLOSE, true))
                || (version == HttpVersion.HTTP_1_0 && request.headers().contains(io.smallvertx.core.http.HttpHeaders.CONNECTION, io.smallvertx.core.http.HttpHeaders.KEEP_ALIVE, true));
    }
    ...
    private void doSendFile(String filename, long offset, long length, Handler<AsyncResult<Void>> resultHandler) {
        synchronized (conn) {
            ...
            if (!contentLengthSet()) {
                putHeader(io.smallvertx.core.http.HttpHeaders.CONTENT_LENGTH, String.valueOf(contentLength));
            }
            if (!contentTypeSet()) {
                ...
                if (li != -1 && li != filename.length() - 1) {
                    ...
                    if (contentType != null) {
                        putHeader(io.smallvertx.core.http.HttpHeaders.CONTENT_TYPE, contentType);
                    }
                }
            }
            prepareHeaders();
    ...
    private boolean contentLengthSet() {
        ...
        return response.headers().contains(io.smallvertx.core.http.HttpHeaders.CONTENT_LENGTH);
    }
    ...
    private boolean contentTypeSet() {
        ...
        return response.headers().contains(io.smallvertx.core.http.HttpHeaders.CONTENT_TYPE);
    }
    ...
    private void prepareHeaders() {
        if (version == HttpVersion.HTTP_1_0 && keepAlive) {
            response.headers().set(io.smallvertx.core.http.HttpHeaders.CONNECTION, io.smallvertx.core.http.HttpHeaders.KEEP_ALIVE);
        } else if (version == HttpVersion.HTTP_1_1 && !keepAlive) {
            response.headers().set(io.smallvertx.core.http.HttpHeaders.CONNECTION, io.smallvertx.core.http.HttpHeaders.CLOSE);
        }
        if (chunked) {
            response.headers().set(io.smallvertx.core.http.HttpHeaders.TRANSFER_ENCODING, io.smallvertx.core.http.HttpHeaders.CHUNKED);
        } else if (keepAlive && !contentLengthSet()) {
            response.headers().set(io.smallvertx.core.http.HttpHeaders.CONTENT_LENGTH, "0");
        }
