...
import io.smallvertx.core.net.impl.*;
...
import io.smallvertx.core.spi.metrics.HttpServerMetrics;
...
public class HttpServerImpl implements HttpServer, Closeable, MetricsProvider {
    ...
    WebSocketServerHandshaker createHandshaker(Channel ch, HttpRequest request) {
        ...
        String connectionHeader = request.headers().get(io.smallvertx.core.http.HttpHeaders.CONNECTION);
    ...
    private void sendError(CharSequence err, HttpResponseStatus status, Channel ch) {
        ...
        if (status.code() == METHOD_NOT_ALLOWED.code()) {
            ...
            resp.headers().set(io.smallvertx.core.http.HttpHeaders.ALLOW, io.smallvertx.core.http.HttpHeaders.GET);
        }
        if (err != null) {
        ...
        protected void doMessageReceived(ServerConnection conn, ChannelHandlerContext ctx, Object msg) throws Exception {
            ...
            if (!DISABLE_WEBSOCKETS) {
                ...
                if (msg instanceof HttpRequest) {
                    ...
                    if (request.headers().contains(io.smallvertx.core.http.HttpHeaders.UPGRADE, io.smallvertx.core.http.HttpHeaders.WEBSOCKET, true)) {
                        // As a fun part, Firefox 6.0.2 supports Websockets protocol '7'. But,
                        // it doesn't send a normal 'Connection: Upgrade' header. Instead it
                        // sends: 'Connection: keep-alive, Upgrade'. Brilliant.
                        String connectionHeader = request.headers().get(io.smallvertx.core.http.HttpHeaders.CONNECTION);
