...
public class DatagramTest extends VertxTestBase {
    ...
    public void testDatagramSocket() throws Exception {
        ...
        TestUtils.assertIllegalArgumentException(() -> peer1.send(Buffer.buffer(), -1, "127.0.0.1", ar -> {
        }));
        TestUtils.assertIllegalArgumentException(() -> peer1.send(Buffer.buffer(), 65536, "127.0.0.1", ar -> {
        }));

        TestUtils.assertNullPointerException(() -> peer1.send((String) null, 1, "127.0.0.1", ar -> {
        }));
        TestUtils.assertIllegalArgumentException(() -> peer1.send("", -1, "127.0.0.1", ar -> {
        }));
        TestUtils.assertIllegalArgumentException(() -> peer1.send("", 65536, "127.0.0.1", ar -> {
        }));

        TestUtils.assertNullPointerException(() -> peer1.send((String) null, "UTF-8", 1, "127.0.0.1", ar -> {
        }));
        TestUtils.assertIllegalArgumentException(() -> peer1.send("", "UTF-8", -1, "127.0.0.1", ar -> {
        }));
        TestUtils.assertIllegalArgumentException(() -> peer1.send("", "UTF-8", 65536, "127.0.0.1", ar -> {
        }));
        TestUtils.assertNullPointerException(() -> peer1.send("", null, 1, "127.0.0.1", ar -> {
        }));

        TestUtils.assertIllegalArgumentException(() -> peer1.sender(-1, "127.0.0.1"));
        TestUtils.assertIllegalArgumentException(() -> peer1.sender(65536, "127.0.0.1"));
        TestUtils.assertNullPointerException(() -> peer1.sender(1, null));

        TestUtils.assertIllegalArgumentException(() -> peer1.listen(-1, "127.0.0.1", ar -> {
        }));
        TestUtils.assertIllegalArgumentException(() -> peer1.listen(65536, "127.0.0.1", ar -> {
        }));
        TestUtils.assertNullPointerException(() -> peer1.listen(1, null, ar -> {
        }));
        TestUtils.assertNullPointerException(() -> peer1.listen(1, "127.0.0.1", null));
    }
