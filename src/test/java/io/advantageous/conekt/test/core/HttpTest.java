...
import io.advantageous.conekt.http.impl.HeadersAdaptor;
import io.advantageous.conekt.parsetools.RecordParser;
...
import static io.advantageous.conekt.test.core.TestUtils.assertIllegalStateException;
...
public class HttpTest extends HttpTestBase {
    ...
    public void testRequestNPE() {
        ...
        assertNullPointerException(() -> client.request(HttpMethod.GET, DEFAULT_HTTP_PORT, DEFAULT_HTTP_HOST, uri, null));
        assertNullPointerException(() -> client.request((HttpMethod) null, DEFAULT_HTTP_PORT, DEFAULT_HTTP_HOST, uri, resp -> {
        }));
        assertNullPointerException(() -> client.requestAbs((HttpMethod) null, "http://someuri", resp -> {
        }));
        assertNullPointerException(() -> client.request(HttpMethod.GET, 8080, "localhost", "/somepath", null));
        assertNullPointerException(() -> client.request((HttpMethod) null, 8080, "localhost", "/somepath", resp -> {
        }));
        assertNullPointerException(() -> client.request(HttpMethod.GET, 8080, null, "/somepath", resp -> {
        }));
        assertNullPointerException(() -> client.request(HttpMethod.GET, 8080, "localhost", null, resp -> {
        }));
    }
    ...
    public void testClearHandlersOnEnd() {
        ...
        server.listen(ar -> {
            assertTrue(ar.succeeded());
            HttpClientRequest req = client.request(HttpMethod.GET, DEFAULT_HTTP_PORT, DEFAULT_HTTP_HOST, path);
    ...
    public void testRequestEnded() {
        ...
        server.listen(ar -> {
            assertTrue(ar.succeeded());
            client.getNow(DEFAULT_HTTP_PORT, DEFAULT_HTTP_HOST, "/blah", resp -> {
                assertEquals(200, resp.statusCode());
                testComplete();
            });
        });
